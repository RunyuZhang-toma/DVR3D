...
end module rotlev3b_size

module rotlev3b_outp
...
end module rotlev3b_outp

module rotlev3b_timing
    integer :: itime0
end module rotlev3b_timing
...
!     compute size of rotational secular problem
 ...
 1000 format(/5x,'Rotational part of rot-vib calculation  with:',&
    ...
    if (.not.zdiag) then
        write(6,"(a100,i4)") 'Hamiltonian construction only requested to be written to stream ISCR =',iscr
        zpham=.false.
    else
        if (zdcore) then
            write(6,"(a100)")'Diagonalisation performed in core using lapack routine dsyev'
        else
    ...
    if (zpvec) write(6,"(a100,f5.2,a20)")'Printing of eigenvector coefficients greater than thresh =', thresh,' requested'
    ...
    if (ztran .and. kmin > 1) write(6,"( 5x,a100,i4)") 'Second set            to be written to stream KVEC2 =',kvec2

    if (.not. zdiag .and. ires==0) write(6,"( 5x,a100,i4)") 'Restart data file one to be written to stream IRF1  =',irf1
 
    if (ires == 0) write(6,"( 5x,a100,i4)") 'Restart data file two to be written to stream IRF2  =',irf2

    if (ires /= 0) write(6,"( 5x,a100,i4)") 'Restart data file two to be read from  stream IRF2  =',irf2


    write(6,"(/5x,'Radau coordinates used')")

    if (ipar == 0) write(6,"(/5x,a100)")'Diatomic assumed homonuclear even parity functions in basis set'

    if (ipar == 1) write(6,"(/5x,a100)")'Diatomic assumed homonuclear odd parity functions in basis set'
...
!     end of file
    if (k2 == 0 .and. kmin == 0) then
        ...
        read(ivec,end=900) k2,maxleg,idvr,lincr
        if (abs(k2) /= 1) then
            write(6,"(//a20,i3,a20,i3,a20)") ' Last block on stream',ivec,' has k =',k2,' 1 expected: STOP'
            stop
        endif
        read(ivec,end=900)
...
!        store the number of functions selected for each k
  ...
  230    continue
        write(6,"(/i10,a50,d20.10,a20,d20.10,a20)") nbass,' functions selected from e =',emin,' to',evibr,' hartree'
    endif
!     store re-start info
...
!     tswalf is the ]xact sum of weights for gauss-jacobi integration
 ...
 1000 format(//i8,' point gauss-associated legendre integration with ',&
            ...
            /(f23.15,d25.12))
    write(6,"(/5x,a50,d26.15,a50,d26.15)") 'computed sum of weights',cswalf,'exact    sum of weights',tswalf
...
! ***** inclusion of the following card is machine dependent *****
        ...
        ip=1-kmin
        if (kmin > 1) ip=k1-1
        write(ilev,"(6i6) ") jrot,ip,0,0,(2-4*ipar),neval
...
60 continue
    write(6,"(//5x,a10,i4,a100,d24.12/)") 'lowest',neval,' eigenvalues in wavenumbers relative to ezero =',ezero
...
!           print largest componants of the eigenvectors
            write(6,"(//'eigenvector componants greater than thresh =',&
                        f5.2)") thresh
            do 80 i=1,neval
            vbig=x0
            ipt=0
...
!     check for compatability
    if (jrot1 /= abs(jrot0)) then
        write(6,"(/5x,a100,i3,a20,i3)") 'j levels mismatched jrot1 =',jrot1,'  jrot0 =',abs(jrot0)
        stop
    endif
    if (kmin1 > kmin0) then
