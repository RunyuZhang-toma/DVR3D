...
!  by GJH & JT 2001.
    ...
    write(6,"(5x,'Partition function: levels read from ilev =',i3/)") ilev
    !201     format(5x,'Partition function: levels read from ilev =',i3/)
    read(ilev,*) j
    read(ilev,*) gz
    gz=gz*autocm
    endif
    write(6,"(5x,'Ground state energy set to',f14.5,' cm-1')") gz
...
1      continue
    ...
    write(6,"(/,i10,a100,i3,i10,a100,I3)") maxr,' transition records read from unit ITRA =',&
    &itra,nr,' selected and      written to unit ITEM =',item
    !1010 format(/,i10,' transition records read from unit ITRA =',i3, &
    !/,i10,' selected and      written to unit ITEM =',I3)
    if (nr<=0) then
    write(6,"(/'No lines selected: stop')")
    ...
    goto 13
    93       continue
    write(6,"(/5x,a20,/i10,a50,i3)") 'List already sorted:',nr,&
    &' transition records found on unit ISPE =',ispe
    endif
...
!     this is the effective main program of spectra
    ...
    if(wmax/=x0) then
    write(6,"(5x,a50,f10.3,a20,f10.3,a100,f10.3/)") 'Frequency range from ',wmin,&
    &' cm-1 to ', wmax,' cm-1 profile half width', dwl
    else
    write(6,"(5x,a50,f10.3,a100,f10.3/)") 'Frequency range from ',wmin,&
    & ' cm-1 to maximum profile half width',dwl
    endif
...
!     calculate the partition function
    ...
    write(6,"(/5x,a29,d12.5,/,a50,d12.5,a10)") 'Partition function Q =',q,&
    & 'estimated error in Q =',qerr,' %'
...
!     output to lineprinter is in wavenumbers and debye.
    ...
    if (zout) write(6,"(//'ipar   j2  p2  i2   j1  p1  i1       e2        e1 freq       s(f-i)'/)")
...
!     first: degeneracy factors
    ...
    write(6,"(/5x,a100,f13.5,a100,f13.5,a10)") 'Ground zero energy           = ',gz,&
    & ' cm-1 Lowest energy in jmax levels = ',emax,' cm-1'
...
!     read in namelist parameters
    ...
    write(6,"(5x,a20,i5,a100,i3)") 'Full line profile requested at',npoints,&
    &' points scatch  IDAT =',idat
    ...
    if (zlist) write(6,"(/5x,a100,d9.2)") 'Linelist printed using relative threshold PRTHR =',prthr
...
!###EDIT EAMON CONWAY... WRITE TO ZLIST ABSOLUTE INTENSITY > XMIN  INSTEAD OF RELATIVE INTENSITY
    ...
    write(ilist,"(i3,2x,2i5,i5,1x,2i5,i5,3f14.6,1x,e16.8,3e15.6)") (iqnum(ir,ic),ic=1,7),(a(ir,ic),ic=1,6),aa


!if (zlist) &
!write(ilist,204) (iqnum(ir,ic),ic=1,7),(a(ir,ic),ic=1,6),aa
    if(a(ir,6)>=prthr) then
    write(6,"(i3,2x,2i5,i5,1x,2i5,i5,3f14.6,1x,e16.8,3e15.6)") (iqnum(ir,ic),ic=1,7),(a(ir,ic),ic=1,6),aa
    i20= i20 + 1
    endif
