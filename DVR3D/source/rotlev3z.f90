...
!     the program works in **** atomic units ***** 
    ...
    write(6,"(5x,'Program ROTLEV3Z (version of May 2002)')")
...
!      integer, ALLOCATABLE, DIMENSION(:) :: indb2
    ...
    read(5,"(6i5)")  nvib,neval,kpar,ibass,iqpar,npnt
...
!     check input data for consistency
    ...
    ierr27=abs(ipar_2-1)
    if (ierr26>0) then
        write(6,"(5x,'ERROR: unit =',i3,2x,'has wrong ipar =',i2)") ivec,ipar_2
        stop
    endif
    if (ierr27>0) then
        write(6,"(5x,'ERROR: unit =',i3,2x,'has wrong ipar =',i2)") ivec1,ipar_1
        stop
    endif
...
!check consistency on all params
 ...
 1200 format(/5x,'Problems with variable',A6,&
    ...
    if (kpar == 0) then
        write(6,"(5x,'Calculate EVEN K block')")
        if (iqpar == 0) then
           write(6,"(5x,'Block K=0 contains EVEN parity functions')")  
        else
           write(6,"(5x,'Block K=0 contains ODD parity functions')")
        endif
    else
        write(6,"(5x,'Calculate ODD K block')")
        if (iqpar == 0) then
            write(6,"(/10x,'Hamiltonian matrix has structure: positive K->q=0')")
            write(6,"(10x,'                                : negative K->q=1')")
        else
            write(6,"(/10x,'Hamiltonian matrix has structure: positive K->q=1')")
            write(6,"(10x,'                                : negative K->q=0')")
        endif
    endif
!
!
...
!      if (nvib/=0) nvib=0
 ...
 1000 format(/5x,'Rotational part of rot-vib calculation  with:',&
    ...
    write(6,"(/5x,a100)")'Diagonalisation performed in core using lapack routine dsyev'

    if (zpvec) write(6,"(5x,a100,f5.2,a20)")'Printing of eigenvector coefficients greater than thresh =' ,&
                &thresh,' requested'
    ...
    ENDIF
    write(6,"(/5x,a100,i4)") 'Hamiltonian scratch file to be held on stream iscr  =',iscr

    if (zpfun) write(6,"(/5x,a100,i4)") 'Eigenvalues    to be written to end of stream ilev  =',ilev
 
    if (zvec) write(6,"(/5x,a100,i4)") 'Eigenvalues & vectors to be written to stream jvec  =',jvec
           

    if (ztran) write(6,"(/5x,a100,i4)") 'Transformed vectors   to be written to stream kvec  =',kvec
...
! get j-th block      
    ...
    write(6,"('(',I3,':',2I1,'): min ',d20.12,' max ',d20.12)")k2,iqa,isa,eviba(1,ia)*conv,eviba(mevala,ia)*conv
! move 1 to  2     
    j=i
    end do
...
! get j-th block      
    ...
    write(6,"('(',I3,':',2I1,'): min ',d20.12,' max ',d20.12)")k2,iqb,isb,evibb(1,ib)*conv,evibb(mevalb,ib)*conv
! move 1 to  2     
    j=i
    end do
!--------------------------------------------
...
! case b: choosing the lowest nbass states ...... (zcut=false)
    ...
    write(6,"(/i10,' Functions selected from E =',d20.10,' to',d20.10,&
            ' cm-1 ')") nbass,emin,emax
...
!     determine  and print basis set labels
    ...
    write(6,"(5x,'(',I3,':',2I1,'):',i5,'/',i5)")kz,iqa,isa,iva(ia),mevala
    else
    write(6,"(5x,'(',I3,':',2I1,'):',i5,'/',i5,' *SATURATED****')")kz,iqa,isa,iva(ia),mevala
    end if
    end do
    do ib=1,nkb
    kz=indk(indb1(ib))
    if (ivb(ib)<mevalb) then
    write(6,"(5x,'(',I3,':',2I1,'):',i5,'/',i5)")kz,iqb,isb,ivb(ib),mevalb
    else 
    write(6,"(5x,'(',I3,':',2I1,'):',i5,'/',i5,' *SATURATED****')")kz,iqb,isb,ivb(ib),mevalb
    end if
    end do  
...
920 read(ivec,end=930)
    i=i+1
    goto 920
930 write(6,"(/'   end of stream ivec =',i4,' at record',i6)") ivec,i
    stop
    end
...
!
    ...
    write(6,"(/5x,'Hamiltonian size =',i6)") ksize
...
!
    ...
    do ia1=1,nka
        ...
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  rot',iva(ia1),iva(ia2)
        else if (k1==k2.and.mn/=0) then
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,' diag',iva(ia1),iva(ia2)
        if (k1==1) & 
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  xdg',iva(ia1),iva(ia2)
        end if
        end do
    end do
    ...
    do ib1=1,nkb
        ...
        mn=mvib1*mvib2
        if (abs(k1-k2)==2.and.mn/=0) then
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  rot',ivb(ib1),ivb(ib2)
        else if (k1==k2.and.mn/=0) then
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,' diag',ivb(ib1),ivb(ib2)
        if (k1==1) & 
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  xdg',ivb(ib1),ivb(ib2)
        end if
        end do
    end do
...
!   coriolis type
        ...
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  cor',iva(ia),ivb(ib)
        end if
!   rot type
        if (abs(k1-k2)==2.and.mn/=0) then
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  rot',iva(ia),ivb(ib)
        end if
! xtra rotational term for k1=k2=1
        if (k1==1.and.k2==1.and.mn/=0) then
        write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,' xrot',iva(ia),ivb(ib)
        end if
...
!-----------------------------------------------------------
    if (k2==1) then !add extra diagonal term ....
        ...
        if (mn/=0) then
    write(6,*)'calculating '
    write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k2,iq2,is2,k2,iq2,is2,'  xdg',mvib2,mvib2
...
! get i-th block      
    ...
    if (mn/=0) then
    write(6,*)'calculating '
    write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  rot',mvib1,mvib2
...
! calculate first coriolis term on its own
    ...
    ALLOCATE(offdg(mn))
    write(6,*)'calculating '
    write(6,"('(',I3,':',2I1,') <-> (',I3,':',2I1,'),   type ...',A5,'. Size:',I5,' x ',I5)")k1,iq1,is1,k2,iq2,is2,'  cor',mvib1,mvib2
    write(6,*)jp1,ip1
!         coefb1=1.d0
!         coefa1=1.d0
    angfac= dble(jjp1)/x4
...
!      nevals=neval
    ...
    IF (INFO /= 0) then
        WRITE(6,"(//5X,'DSPEVX RETURNED INFO =',I3)") INFO
        WRITE(96,"(//5X,'DSPEVX RETURNED IFAIL =',I3)") IFAIL
        stop
    END IF
...
!     print eigenvalues in atomic units & wavenumbers
    ...
    write(6,"(//5x,'lowest',i4,' eigenvalues in hartrees',/)") nevalp
    write(6,"(5d24.12/)") (eval(i),i=1,nevalp)
    if (zpfun) then
        if (k1 == 0) then
...
! ***** inclusion of the following card is machine dependent *****
        ...
        ip=mod(jrot+kpar,2)
        write(ilev,"(6i6)") jrot,ip,0,0,(2-4*iqpar),nevalp
...
!        write eigenvalues, eigenvectors, etc to stream jvec
   ...
   60 continue
    write(6,"(//5x,a10,i4,a100,d20.12/)")'lowest', nevalp,&
                &' eigenvalues in wavenumbers relative to ezero =',ezero

    write(6,"(5d24.12/)") (evalcm(i),i=1,nevalp)
...
!             print complete eigenvectors
 ...
 !1030         format(//'    eigenvectors',/)
              do 70 i=1,neval
              write(6,"(/(1x,10f13.7))") (evec(j,i),j=1,ibass)
 !1040         format(/(1x,10f13.7))
   70         continue
        else
!             print largest components of the eigenvectors
              write(6,"(a100,f5.2)") 'eigenvector components greater than thresh =',thresh
! 1050         format(//'eigenvector components greater than thresh =',&
   ...
   90         continue
              write(6,"('  vector',i3,5(i7,f14.10)/(11x,5(i7,f14.10)))") i,(ibig(j),evec(ibig(j),i),j=1,max(1,ipt))
...
!     read rotlev header
    ...
    nval=min(nval,neval)
    IF (ZPTRA) WRITE(6,"(/5X,'J =',I3,' KMIN =',I2,'  PARITY =',I2,' NVAL =',I3/)") JROT0_1,KMIN0_1,IPAR_1,NVAL
!    size of final basis?
    nend=0
...
!        wt(l)=wt(l)/(sqrt(2.d0)*beta*y(l)**(ALF+1.d0))
    ...
    if (myid == 0) WRITE(6,"(F23.15,D25.12,F13.5)") Y(l),WT(l),R(l)

    IF ((R(I) < X0) .and. (myid == 0)) WRITE(6,"(5X,a100,I3,a100)") '***** WARNING: FOR INTEGRATION POINT',&
    &I,', R LESS THAN ZERO *****'
...
!     CHECK THAT THE CORRECT POINTS & WEIGHTS HAVE BEEN GENERATED
 ...
 1020 FORMAT(/5X,'COMPUTED SUM OF POINTS',D26.15,' & WEIGHTS',D26.15,&
    ...
    RETURN
900 if (myid == 0) WRITE(6,"(//5X,'(ERROR) POINTS & WEIGHTS IN ERROR, ADJUST ALGORITHM',//)")
    STOP
    END
!####################################################################
!#######################################################################
