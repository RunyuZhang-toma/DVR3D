...
!     compute size of rotational secular problem
 ...
 !1010 format(/5x,'Title:',9a8)
      if (.not.zdiag) then
         write(6,"(/5x,a100,i4)") 'Hamiltonian construction only requested to be written to stream ISCR =',iscr
...
! 1043     format(5x,'Printing of transformed vectors not requested')
      ...
      if (ires /= 0) write(6,"(/5x,a100,i4)") 'DVR3DRJZ data  to be read         from stream IRTF1  =',irf1
...
! 1063 format( 5x,'Restart data file two to be read from  stream ',&
 ...
 !1080 format(/5x,'Radau coordinates used')
      if (ipar == 0) write(6,"(a100)")'Diatomic assumed homonuclear even parity functions in basis set'
 ...
 !1110 format(a100)'Diatomic assumed homonuclear odd parity functions in basis set'
...
!     end of file
      if (k2 == 0 .and. kmin == 0) then
         ...
         read(ivec,end=900) k2,maxleg,idvr,lincr
         if (abs(k2) /= 1) then
            write(6,"(a100,i3,a20,i3,a20)") ' Last block on stream',ivec,' has k =',k2,' 1 expected: STOP'
...
!     tswalf is the ]xact sum of weights for gauss-jacobi integration
 ...
 1000 format(//i8,' point gauss-associated legendre integration with ',&
             'k =' ,i3//5x,'integration points',11x,'weights',&
              /(f23.15,d25.12))
      write(6,"(a100,d26.15,a100,d26.15)")'computed sum of weights', cswalf,'exact    sum of weights',tswalf
...
!        write eigenvalues, eigenvectors, etc to stream jvec
  ...
  100    continue
   ...
   60 continue
      write(6,"(//5x,a10,i4,a100,d24.12/)") 'lowest',neval,&
...
! ***** inclusion of the following card is machine dependent *****
 ...
 !1026    format(4d20.12)
   ...
   60 continue
      write(6,"(//5x,a100,i4,a100,d24.12/)")'lowest', neval,' eigenvalues in wavenumbers relative to ezero =',ezero
 !1010 format(//5x,'lowest',i4,' eigenvalues in wavenumbers relative to',&
  !                ' ezero =',d24.12/)
      write(6,"(5d24.12/)") (energy(i),i=1,neval)
 !1020 format(5d24.12/)
