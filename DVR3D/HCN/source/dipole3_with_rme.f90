...
!
      ...
      common /logic/ zmors1,znco1,znco2,zprint,zpmin,ztra,zstart,zmors2,zrme1,zrme2,zrme3,zuvvis,zpseg
...
!     spectrum, itra.
!
      common /logic/ zmors1,znco1,znco2,zprint,zpmin,ztra,zstart,zmors2,zuvvis,zpseg,zrme1,zrme2,zrme3
      common /stream/ iket, ibra, itra, iscr, ires, mblock, nblock
      data zmors1/.true./, zprint/.false./, ztra/.true./,&
           ...
           zrme1/.true./, zrme2/.true./, zrme3/.false./
      end
...
print*,"ZPSEG:",zpseg
      if (zpseg==.true.) then 
         open(unit=iket,form='unformatted',recordtype='segmented')  ; print*, "test1"!* 
      else 
        open(unit=iket,form='unformatted'); print*, "test2"
      end if 
...
997          format(//,5x,'** fatal ** parity mismatch, spin forbidden',&
         ...
         if (jrot1 .eq. jrot2) then
            ...
            if (ipar1.eq.ipar2) then
             if((zrme2 .eq. .false.) .and.(zrme3 .eq. .false.) )  write(6,997) ipar1,ipar2
             if((zrme2 .eq. .false.) .and.(zrme3 .eq. .false.) )  stop
            endif
         endif
      endif
      ipar= ipar1
...
!
      read(5,*) npot,nv1,nv2,ibase1,ibase2
101   format(5i5)
...
!
    ...
    if( (zrme2 .eq. .false.) .and.(zrme3 .eq. .false.) ) write(6,9999)
...
if((kmin1 .eq. 0) .and. (kmin2 .eq. 0)) then
k2= abs(kk2)
else
k2= abs(kk2) + kmin2
end if
...
if((k2 .gt. jk2) ) go to 98

if(kk2 .gt. jrot2) go to 98
...
if(k2 .lt. 0) go to 110
if(kk2 .gt. jrot2) go to 110
if((k2 .gt. jk2) ) go to 110
...
if (order .eq. 3) then ! OCTUPOLE

    if(nu .eq. -3 ) then 
        if(abs(kk1) .gt. abs(nu)) then
...
else if (order .eq. 2) then !QUADRUPOLE
    if(nu .eq. -2 ) then 
...
203   format(5x,'j1=',i4,'  kmin1=',i4,'  j2=',i4,'  kmin2=',i4,&
      ...
      ezero=x0
      write(6,*) "DVR3D is considering electronic transitions?", zuvvis
       if (zuvvis==.true.) then 
	         read(5,*) ezero,ezeroup,te
                 write(6,*) "ZPE gs=",ezero,", ZPE exc=",ezeroup,", Te=", te 
       else
                 read(5,*) ezero
      end if
  505 format(f20.0)
...
if (zuvvis==.true.) then
      ...
      write(6,206) ie1,ie2,xe1,xe2(ie2)*autocm,dd,tzd,txd,t,sxd,a
...
!--------------------
2060  format(2(i4),3(3x,f10.3),5(2x,e13.6))
2002  continue
end if
