...
module size
   save
     integer :: iqpar     ! number of dvr points in each radial coordinate
     integer :: nktot   ! size of vibrational radial problem (even basis)
     ...
     integer :: kpar     ! zrot=t, kmin=1 sym. rot. basis, =0 anti-sym.
     ...
     integer :: idia
     integer :: nr     ! maximum dimension of theta dvr grid used in vibrational problem
     integer :: iang     ! maximum number of discrete angles retained in vib. problem
     integer :: mxblk_odd   ! size of vibrational radial problem (odd  basis)
     integer :: ndvr   ! maximum size of vibrational problem (excluding linear geom)
end module size


module outp
   save
     integer :: idiag           ! stream for final eigenvalues (formatted).
     ...
     integer :: ithre          ! holds output second set                        zvec=.true.
     integer :: kvecpb            ! holds output first  set transformed vectors used if ztran=.true.
     integer :: kvec           ! holds output second set   used if ztran=.true.
     integer :: ilev           ! scratch file used for restart runs
                                   ! holds hamiltonian file used if always

     integer :: iscr           ! = 0  normal run
     ...
     integer :: nploti           ! irf1   restart file one  used if zdiag=.false.
     integer :: nplotf          ! irf2   restart file two  used if always
     ...
     logical :: zplot     ! T z axis is along r2, = f z axis is along r1.
...
!     elements
      ...
      implicit none
      double precision :: x0,xp5,x4,sqrt2
      xp5 = 0.5d0
      x1 = 4.0d0
...
!     by using some lapack routines if they work.
      ...
      implicit none
      double precision :: x0,autocm
      autocm = 2.19474316d+05
